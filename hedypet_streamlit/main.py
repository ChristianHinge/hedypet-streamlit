...
# The 'with col1:' is removed, and this code is no longer indented
fig = go.Figure()
...
if len(dynamic_vars) > 2:
    ...
    linestyle_map = {val: linestyles[i % len(linestyles)] for i, val in enumerate(linestyle_values)}
...
# st.write("---") # Adds a horizontal rule for separation
# st.header("Filtered Data Statistics")

# with st.expander("Expand to see the data table"):
#     # Prepare the DataFrame for display
#     display_df = summary_stats.copy()
    
#     # Define the desired column order, using the legend_order list we already created
#     column_order = legend_order + ['t', 'mean', 'std', 'count']
    
#     # Reorder the columns
#     display_df = display_df[column_order]

#     # Use st.dataframe to show the data.
#     # The .style.format() method makes the numbers easier to read.
#     st.dataframe(
